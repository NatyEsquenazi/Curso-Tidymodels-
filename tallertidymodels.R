...
pinguinos_db %>% ggplot(aes(x=sexo, y=masa_corporal_g, fill=sexo)) + 
                        geom_boxplot() +
                        facet_wrap(~especie+isla) +
                        geom_signif(comparisons = list(c("macho", "hembra")), 
                        map_signif_level=TRUE,
                        test = "t.test") # Significancia estadistica 
# Correlación entre variables numéricas -----------------------------------
pinguinos_db %>% 
  select(-especie,-sexo,-isla) %>% 
  corrr::correlate() 

## Rta: la masa corporal correlaciona mucho con el alto de la aleta
...
set.seed(1234)
division      <-  initial_split(data = pinguinos_db, prop = .8) #prop dice que separamos 80% y 20%
...
# Estratificar datos ------------------------------------------------------
set.seed(1234)
division_strat<-  initial_split(data = pinguinos_db, prop = .8, strat = sexo) # estratificamos por sexo para que este balanceado
...
masa_recipe <-recipe(masa_corporal_g ~ ., data = entrenamiento) %>% 
              ...
              prep() #es para usar las funciones bake y juice
...
modelo_lineal <- linear_reg() %>% #tipo de regresion
                 set_engine("lm") %>% #motor
                 set_mode("regression") #modo


modelo_rf     <- rand_forest() %>% # random forest
                 set_engine("ranger") %>% 
                 set_mode("regression") 

# Modelos -----------------------------------------------------------------

translate(modelo_rf) #forma de hacer summary
